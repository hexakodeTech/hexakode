...
import { PortfolioViewTracker } from "@/components/common/AnalyticsTracker";
...
export default async function ProjectDetailPage(props: { params: Promise<{ slug: string }> }) {
  ...
  const galleryImages = rawGallery
    .map((img: unknown, idx: number) => {
      ...
      const url = (item?.imageUrl || item?.url || "") as string;
      const alt = (item?.alt || `Gallery image ${idx + 1}`) as string;
      const order =
