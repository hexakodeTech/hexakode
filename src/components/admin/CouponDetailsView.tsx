...
export default function CouponDetailsView({ code }: CouponDetailsViewProps) {
  ...
  const handleExportPDF = async () => {
    const tableHeaders = ['Name', 'Company', 'Project Type', 'Date'];
    ...
    const summaryItems = [
      { label: 'Referral Code', value: coupon.code },
      { label: 'Active Days', value: coupon.activeDays ? `${coupon.activeDays} Days` : 'Infinite' },
      { label: 'Maximum Enquiry Limit', value: String(coupon.maxLimit) },
      { label: 'Current Enquiries', value: String(coupon.currentEnquiries) },
      { label: 'Remaining Enquiries', value: String(coupon.remainingEnquiries) },
      { label: 'Created Date', value: coupon.createdDate },
      { label: 'Expiry Date', value: coupon.expiryType === 'infinite' ? 'Never' : (coupon.expiryDate || 'Never') }
    ];

    const metadata = [
      { label: 'Generated By', value: 'HexaKode Admin' },
      { label: 'Generated On', value: new Date().toLocaleString() }
    ];

    toast.promise(
      exportToPDF({
        filename: `Referral-Code-${coupon.code}.pdf`,
        title: 'Referral Code Report',
        metadata,
        summaryTitle: 'Referral Program Summary',
        summaryItems,
        tableHeaders,
        tableData,
        emptyMessage: 'No enquiries found for this referral code.'
      }),
      {
