...
export default function CouponDetailsView({ code }: CouponDetailsViewProps) {
  ...
  if (isLoading) {
    return (
      <div className="flex h-64 flex-col items-center justify-center gap-2">
        ...
        <span className="text-xs text-on-surface-variant/70">Loading coupon details...</span>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="text-center py-12">
        <h3 className="font-headline-md text-primary mb-2">Coupon Not Found</h3>
        <p className="text-xs text-on-surface-variant mb-6">The requested coupon code does not exist.</p>
        <Link
          href="/admin/coupons"
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-headline-sm text-sm font-semibold text-primary">
            Coupon: <span className="font-mono">{coupon.code}</span>
          </h1>
          <p className="text-xs text-on-surface-variant/60 mt-0.5">
            Detailed tracking metrics and enquiries using this promotional code
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-xl p-5 shadow-card flex items-start gap-4">
          ...
          <div>
            ...
            <span
              className={`text-[8px] font-semibold px-2 py-0.5 rounded-full uppercase mt-1 inline-block ${
                coupon.status === 'ACTIVE'
                  ? 'bg-secondary-container/20 text-on-secondary-container'
                  : coupon.status === 'LIMIT REACHED'
        ...
        <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-xl p-5 shadow-card flex items-start gap-4">
          ...
          <div>
            <span className="font-label-mono text-[9px] uppercase tracking-wider text-on-surface-variant/60">
              Lifespan ({coupon.activeDays} Days)
            </span>
            <p className="text-xs font-semibold text-primary mt-1">
              Expires: <span className="font-mono">{coupon.expiryDate}</span>
            </p>
      ...
      <DataTable
        title="Enquiries Using This Coupon"
        subtitle={`Live contact form enquiries submitted with code ${coupon.code}`}
        searchValue={search}
        onSearchChange={(val) => {
          setSearch(val);
          setCurrentPage(1);
        ...
        {displayedEnquiries.length === 0 ? (
          <tr>
            <td colSpan={7} className="text-center py-8 text-xs text-on-surface-variant/50">
              No enquiries registered with this coupon code.
            </td>
          </tr>
        ) : (
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 pt-2 text-xs">
                ...
                <div>
                  ...
                  <span className="font-mono font-semibold text-secondary">{activeEnquiry.couponCode}</span>
                </div>
              </div>
